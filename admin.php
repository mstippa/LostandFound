...
# checks if user is posting information
if(array_key_exists("submit", $_POST)) {    
  ...
  if($error != "") {
      ...
      # hashes the password that the user posts
      $temp = md5($_POST['password']);
      # creates a query that searches the users table for the username and password that the user input
      ...
      # fetches the result set of query
      $row = mysqli_fetch_array($result);
      
      if(isset($row)) {
        # sets the session id as the hashed username 
        ...
        header("Location: /admin/admin2.php");
        # gets here if user inputs incorrect username or password
      } else
